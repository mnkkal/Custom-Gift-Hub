...
const default_collection_filters_1 = require("@vendure/core/dist/config/catalog/default-collection-filters");
...
async function run() {
    ...
    const superAdminUser = await app.get(core_1.ChannelService)['connection'].rawConnection.getRepository(core_1.User).findOneOrFail({
    ...
    let categoryFacet = await facetService.findByCode(ctx, 'category', core_1.LanguageCode.en);
    if (!categoryFacet) {
    ...
    for (const item of CATEGORY_FACETS) {
        ...
        if (!fv) {
            fv = await facetValueService.create(ctx, categoryFacet, {
    ...
    for (const item of CATEGORY_FACETS) {
        ...
        if (collection && fv) {
            ...
            await collectionService.update(ctx, {
                ...
                filters: [
                    {
                        code: default_collection_filters_1.facetValueCollectionFilter.code,
                        arguments: [
    ...
    for (const col of updatedCollections.items) {
        await collectionService.applyCollectionFiltersInternal(col, false);
